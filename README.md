...
> 출처 : https://www.newswire.co.kr/newsRead.php?no=913257&sourceType=mail
...
2) 두 학기 이상을 수강한 학생 중 만나이 변동으로 인해, 두 학기 이상을 수강한 학생 행 제외 & 나이 값이 2개인 학생은 과거의 값(이용자의 초기 가입 나이)만 선택


## 6.2 Target 칼럼 생성 
- fianl_result 칼럼의 값 `Withdrawn(탈퇴)` / `Fail(낙제), Pass(통과), Distinction(우수 수료)` 중 Withdrawn(탈퇴)를 이탈로 간주
...
|----------------------------------|----------------------------------|
| ![수행결과 이미지](image/outlier.png) | ![수행결과 이미지](image/outlier_removed.png) |
...
<img width="350" height="153" alt="image" src="https://github.com/user-attachments/assets/a9880e15-ca0d-4a5e-89de-680c68d82f44" />
<img width="927" height="456" alt="image" src="https://github.com/user-attachments/assets/d8bbb823-801a-40e7-9928-4e489e27e87c" />
...
# 9. 한계점 및 보완점
## 9-1. 한계점
- 성능 향상을 위해 생성한 파생변수가 오히려 노이즈로 작용한 것 같음 -> 적절한 파생변수 생성 필요
- 온라인 학습 플랫폼 특성상 가입 시점에는 고객의 기본 정보가 제한적으로 수집되는 한계 -> 초기 단계에서 고객의 단순한 인구통계학적 특성만으로는 이탈 가능성 예측 어렵
- 사용자의 학습 패턴, 접속 빈도, 콘텐츠 선호도 등 행동 기반 데이터가 쌓인 이후에야 이탈 징후를 보다 명확히 포착할 수 있는 구조 -> 가입 직후보다는 일정 기간의 활동 데이터를 확보한 이후에 예측 성능이 극대화되는 경향

<br>

## 9-2. 보완점
- 학생들이 이 플랫폼에 가입할 당시에 가져올 수 있는 학생들의 과거 행동 데이터를 추가하면 모델 성능이 올라갈 것으로 기대
